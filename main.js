...
(function () {
  ...
  /* ---------- Hero sparkles (home only) ----------
     ...
     stars blink around the hero accent word on short staggered loops,
     each jumping to a fresh spot every cycle. Skipped entirely under
     reduced motion; no-JS pages simply never get them. */
  var sparkleHost = document.querySelector(".hero h1 em");
  if (sparkleHost && !reducedMotion) {
    ...
    var placeSparkle = function (el) {
      el.style.left = (Math.random() * 110 - 5).toFixed(1) + "%";
      el.style.top = (Math.random() * 130 - 40).toFixed(1) + "%";
